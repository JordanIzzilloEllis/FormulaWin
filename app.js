let apiResponse = fetch('http://localhost:4000/requiredInfo')
    ...
    .then(formattedData => {
        let driverArray = [];
        for (i = 0; i < formattedData['driverData'].length; i++) {
            driverArray.push(formattedData['driverData'][i])
        }
        let raceArray = formattedData['raceData']
        dispDriverInfo(driverArray)
        dispRaceInfo(raceArray)
        return driverArray
    })

function dispRaceInfo(raceData) {
    ...
    const raceName = document.createTextNode(`Round ${raceData['0']['round']}: The ${raceData['0']['name']}`);
    ...
    const circuit = document.createTextNode(`${raceData['0']['circuit']}`);
    ...
    const date = document.createTextNode(`${raceData['0']['date']}`);
    ...
    trackMapImage.src = `Circuits/${raceData['0']['circuit'].trim()}.png`
...
function sortDriverArray(theArray, sortType) {
    if (sortType === 'Finished') {
        let sorted = theArray.sort((a, b) => (a.position.localeCompare(b.position, undefined, {
            numeric: true,
            sensitivity: 'base'
          })));
        return sorted;
